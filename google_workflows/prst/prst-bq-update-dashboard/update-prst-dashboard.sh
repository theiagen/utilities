...
# initialize log
echo "DEBUG: Sending ${ORIGIN_TABLE_NAME} to ${DEST_BUCKET_URI}" | tee -a automation_logs/${LOG_FILE}
...
if [ "${OUTPUT_FILENAME_PREFIX}" == "date" ]; then
  echo 'DEBUG: User specified "date"' "for OUTPUT_FILENAME_PREFIX, final output JSON will be named: ${DEST_BUCKET_URI}${ORIGIN_TABLE_NAME}_${DATE_TAG}.json" | tee -a automation_logs/${LOG_FILE}
  OUTPUT_FILENAME_PREFIX="${DEST_TABLE_ID}_${DATE_TAG}"
else
  echo "DEBUG: User did not specify 'date' for OUTPUT_FILENAME_PREFIX, final output JSON will be named: ${DEST_BUCKET_URI}${ORIGIN_TABLE_NAME}.json" | tee -a automation_logs/${LOG_FILE}
  OUTPUT_FILENAME_PREFIX="${ORIGIN_TABLE_NAME}"
fi

# download Terra table TSV using export_large_tsv.py from Broad
echo "DEBUG: downloading Terra table (${ORIGIN_TABLE_NAME}) from ${WORKSPACE_NAME}" | tee -a automation_logs/${LOG_FILE}
...
# add new column
echo "DEBUG: adding new column" | tee -a automation_logs/${LOG_FILE}
sed -i "s/$/\t${DEST_TABLE_ID}/" "${DEST_TABLE_ID}_${DATE_TAG}.tsv"

# rename header to 'source_terra_table'
echo "DEBUG: renaming header" | tee -a automation_logs/${LOG_FILE}
sed -i "1{s/${DEST_TABLE_ID}$/source_terra_table/}" "${DEST_TABLE_ID}_${DATE_TAG}.tsv"

# convert TSV to newline-delimited JSON
echo "DEBUG: converting TSV to newline-delimited JSON" >> automation_logs/${LOG_FILE}
python3 /utilities-0.2/scripts/tsv_to_newline_json.py "${DEST_TABLE_ID}_${DATE_TAG}.tsv" "${OUTPUT_FILENAME_PREFIX}"

# copy to GCS bucket
echo "DEBUG: copying to GCS bucket" | tee -a automation_logs/${LOG_FILE}
gsutil -m cp "${OUTPUT_FILENAME_PREFIX}.json" "${DEST_BUCKET_URI}${OUTPUT_FILENAME_PREFIX}.json"

echo "DEBUG: removing TSV and JSON files" | tee -a automation_logs/${LOG_FILE}
rm -v "${DEST_TABLE_ID}_${DATE_TAG}.tsv"
rm -v "${OUTPUT_FILENAME_PREFIX}.json"

echo "DEBUG: loading to big query" | tee -a automation_logs/${LOG_FILE}
bq load --ignore_unknown_values=true --project_id=general-theiagen --replace=true --source_format=NEWLINE_DELIMITED_JSON pr_sci_trust_dashboard_processing.${OUTPUT_FILENAME_PREFIX} ${DEST_BUCKET_URI}${OUTPUT_FILENAME_PREFIX}.json bq_schema/schema_PRST_v1.json

echo "DEBUG: done" | tee -a automation_logs/${LOG_FILE}